...
3. Para **cristo-redentor**, `refreshCristoRedentorSnapshot` (`src/lib/server/cristoRedentorSnapshot.ts`) ejecuta en paralelo `fetchConsolidado(29)`, `fetchDetailHTML(29, "Sistema-Cristo-Redentor")`, `fetchClima`, **`fetchMendozaForecast`** (Prensa Mendoza), luego `parseForecastFromHTML`, `extractAlertsFromDetailHTML` y **`mapToSnapshot`** (`src/lib/server/passMapper.ts`); el boletín queda en `extendedForecastText` del JSON y en la vista como texto plano para la inferencia cruzada.
...
- **`inferPassStatus`** (`src/utils/inferPassStatus.ts`): si `view.operationalInfo.rawStatus` tiene texto, entra en **`inferFromOfficialApi`** y **no** usa la rama legacy basada solo en horario/alertas del HTML antiguo.
- **Prioridad del `rawStatus` oficial** para CERRADO y CONDICIONADO: si `rawStatus` (normalizado) es `CERRADO` o `CONDICIONADO`, el resultado es cerrado o condicionado con confianza alta, usando `motivo` si existe.
- Si **`rawStatus === "ABIERTO"`** (caso frecuente de desfase del JSON de Gendarmería frente a Vialidad o a la provincia):
  - **Fusión de fuentes (`fuentes_cruzadas`):** si hay **corte total** en `vialidadEstado` y, en un corpus unificado (`vialidadObservaciones`, `htmlAlerts`, boletín Prensa Mendoza en texto plano), aparece un **cierre explícito del paso o del tránsito** (p. ej. «Paso cerrado desde…», «cierre del tránsito en Uspallata»), el estado inferido pasa a **cerrado** aunque el API siga en ABIERTO. El boletín de [Prensa Gobierno de Mendoza](https://prensa.mendoza.gob.ar/estado-del-tiempo-y-pasos-internacionales/) se expone en la vista como `supplementaryBulletinPlain` (derivado de `extendedForecastText` en el snapshot) y, para este slug, una línea tipo «Paso Cristo Redentor: Cerrado…» también fuerza **cerrado** si contradice al API.
  - Si no aplica lo anterior pero **`vialidadEstado`** es corte total o restricción fuerte sin texto de cierre inequívoco, se mantiene **condicionado** con **`displayLabel: "ABIERTO"`** (alerta de Vialidad sin elevar a cierre).
  - En la **card de la home** (`PasoCard.astro`), no se aplica estilo «abierto» cuando hay corte total de vialidad aunque el badge lleve ABIERTO por condicionado.
  - El **`schedule`** se usa **solo** para **`closesInMinutes`** cuando el estado inferido sigue siendo abierto o condicionado; no fuerza “cerrado fuera de horario” si el API sigue en ABIERTO.

## Problemas identificados

- **API «ABIERTO» vs realidad (Vialidad / provincia):** el consolidado de [Argentina.gob.ar](https://www.argentina.gob.ar/seguridad/pasosinternacionales/detalle/ruta/29/Sistema-Cristo-Redentor) puede ir **horas** detrás del HTML de detalle, de Vialidad o del boletín de Mendoza. La fusión de fuentes en `inferPassStatus` mitiga el badge verde engañoso; igual conviene cruzar siempre con la fuente que actualice último en el terreno.
