...
export function statusAccentClass(status: PassStatus): string {
  switch (status) {
    case "open":
      return "text-status-open ring-status-open/30 bg-status-open/10";
    case "closed":
      return "text-status-closed ring-status-closed/25 bg-status-closed/10";
    case "conditional":
      return "text-status-conditional ring-status-conditional/30 bg-status-conditional/10";
    default:
